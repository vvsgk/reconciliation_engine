    ...
    void concurrentDuplicateStorm() throws InterruptedException {
        ...
        for (int i = 0; i < threads; i++) {
            results.add(ex.submit(() -> {
                latch.await();
                try {
                    eventService.processEvent(new EventRequest("DUP-CONC","2026-08-15T09:00:00Z".equals("")?null:Instant.parse("2026-08-15T09:00:00Z"), "ACC-CONC", new BigDecimal("100.00"), "USD", "stress"));
                    return true;
                } catch (Exception e) {
                    return false;
                }
